...
static inline void
dsaf_write_sub(struct dsaf_device *dsaf_dev, u32 reg, u32 val)
{
...
static inline u32 dsaf_read_sub(struct dsaf_device *dsaf_dev, u32 reg)
{
	u32 ret;
...
void hns_dsaf_xge_srst_by_port(struct dsaf_device *dsaf_dev, u32 port, u32 val)
{
	...
	if (IS_ENABLED(CONFIG_OF) && dsaf_dev->dev->of_node) {
		...
		if (!HNS_DSAF_IS_DEBUG(dsaf_dev))
			...
			reg_val |= 0x2082082 << (dsaf_dev->reset_offset + 5);
...
void hns_dsaf_ge_srst_by_port(struct dsaf_device *dsaf_dev, u32 port, u32 val)
{
	...
	if (IS_ENABLED(CONFIG_OF) && dsaf_dev->dev->of_node) {
		if (!HNS_DSAF_IS_DEBUG(dsaf_dev)) {
			...
			reg_val_1 = 0x15540 << (dsaf_dev->reset_offset - 1);
			reg_val_2 = 0x100 << (dsaf_dev->reset_offset - 1);
...
void hns_ppe_com_srst(struct ppe_common_cb *ppe_common, u32 val)
{
	...
	if (!HNS_DSAF_IS_DEBUG(dsaf_dev)) {
		...
		reg_val = 0x100 << (dsaf_dev->reset_offset - 1);
...
phy_interface_t hns_mac_get_phy_if(struct hns_mac_cb *mac_cb)
{
	...
	if (IS_ENABLED(CONFIG_OF) && mac_cb->dev->of_node) {
		if (is_ver1 && (mac_id >= 6 && mac_id <= 7)) {
			...
			reg = is_ver1 ? HNS_MAC_HILINK4_REG :
			...
			reg = is_ver1 ? HNS_MAC_HILINK3_REG :
		...
		phy_if = (phy_interface_t)obj->integer.value ?
...
#define RX_CSR(lane, reg) ((0x4080 + (reg) * 0x0002 + (lane) * 0x0200) * 2)
		...
		hns_dsaf_acpi_srst_by_port(dsaf_dev, HNS_OP_SERDES_LP_FUNC, 0,
					   mac_cb->mac_id, !!en);
	}
