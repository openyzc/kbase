...
static int
hns_mac_parse_addr(struct device *dev, struct fwnode_handle *fwnode)
{
	u32 addr;
	int ret;
...
static int
hns_mac_register_phy(struct mii_bus *mdio, struct fwnode_handle *child,
		     u32 addr)
{
	...
	rc = fwnode_property_read_string(child, "phy-mode", &phy_type);
	if (rc < 0)
		return rc;
...
static void hns_mac_miibus_register(struct hns_mac_cb *mac_cb)
{
	...
	addr = hns_mac_parse_addr(mac_cb->dev, mac_cb->fw_port);
	if (addr < 0)
		return;

	/* dev address in adev */
	dev = ACPI_DEV(to_acpi_device_node(acpi_fwnode_handle(args.adev)));
	mii_bus = dev_get_drvdata(dev);
	rc = hns_mac_register_phy(mii_bus, mac_cb->fw_port, addr);
	if (!rc) {
		mac_cb->phy_fwnode = mac_cb->fw_port;
			    ...
			    struct device_node *np)
{
	...
	if (is_of_node(mac_cb->fw_port)) {
		...
		hns_mac_miibus_register(mac_cb);
	} else {
...
int hns_mac_get_cfg(struct dsaf_device *dsaf_dev, struct hns_mac_cb *mac_cb)
{
	...
	ret = hns_mac_get_info(mac_cb, mac_cb->dev->of_node);
	if (ret)
		return ret;
...
int hns_mac_init(struct dsaf_device *dsaf_dev)
{
	...
	int ret;
	u32 port_id;
	u32 max_port_num = hns_mac_get_max_port_num(dsaf_dev);
	struct hns_mac_cb *mac_cb;
	...
	/* if don't get port subnode from dsaf node
